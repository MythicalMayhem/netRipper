...
while (os.path.isdir(path) != True): 
    path = input('choose a path for wifi log files (press enter to choose the same direcory as the .exe file) :')
    if (os.path.isdir(path) == True):
        break
    elif path == '':
        absolute_path = os.path.abspath(__file__)
        path = os.getcwd()
        print("Path : " + path +"/"+name+ '/wifilyzer')
